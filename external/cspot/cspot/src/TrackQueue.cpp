...
static std::chrono::steady_clock::time_point
    spotify_cdn_retry_after =
        std::chrono::steady_clock::time_point::min();
void QueuedTrack::stepLoadCDNUrl(
    const std::string& accessKey
) {
  const auto now =
    std::chrono::steady_clock::now();

if (now < spotify_cdn_retry_after) {
  return;
}
  if (accessKey.empty()) {
    // Auf Access-Key warten
    return;
  }

  CSPOT_LOG(
      info,
      "Received access key, fetching CDN URL..."
  );

  try {
    const std::string fileIdHex =
        bytesToHexString(fileId);

    const std::string requestUrl =
        string_format(
            "https://api.spotify.com/v1/storage-resolve/"
    ...
    if (!req) {
      throw std::runtime_error(
          "HTTPClient returned null response"
      );
    }
...
if (result.find("\"status\": 429") != std::string::npos ||
    result.find("\"status\":429") != std::string::npos) {
  ...
  CSPOT_LOG(
      error,
      "Spotify CDN Resolve Rate-Limit 429. Neuer Versuch fruehestens in 60 Sekunden"
  );
  ...
  return;
    ...
    if (result.empty()) {
      throw std::runtime_error(
          "CDN Resolve returned empty response"
      );
    }
    ...
    if (!cJSON_IsArray(cdnUrlArray) ||
        cJSON_GetArraySize(cdnUrlArray) == 0) {

      cJSON_Delete(jsonResult);

      throw std::runtime_error(
          "CDN Resolve JSON contains no cdnurl array"
      );
    }
    ...
    if (!cJSON_IsString(firstUrl) ||
        firstUrl->valuestring == nullptr) {

      cJSON_Delete(jsonResult);

      throw std::runtime_error(
          "CDN Resolve cdnurl entry is invalid"
      );
    }

    cdnUrl =
        firstUrl->valuestring;
    ...
    if (!jsonResult.contains("cdnurl")) {
      throw std::runtime_error(
          "CDN Resolve JSON contains no cdnurl"
      );
    }

    const auto& cdnUrls =
        jsonResult.at("cdnurl");

    if (!cdnUrls.is_array() ||
        cdnUrls.empty() ||
        !cdnUrls.at(0).is_string()) {

      throw std::runtime_error(
          "CDN Resolve cdnurl is not a valid URL array"
      );
    }
    ...
    if (cdnUrl.empty()) {
      throw std::runtime_error(
          "CDN Resolve returned empty CDN URL"
      );
    }
    ...
    CSPOT_LOG(
        error,
        "Cannot fetch CDN URL: unknown exception"
    );
