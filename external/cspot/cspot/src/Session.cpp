...
{
    auto apResolver =
        std::make_unique<ApResolve>("");

    auto connection =
        std::make_unique<cspot::PlainConnection>();

    connection->timeoutHandler = [this]() {
        return this->triggerTimeout();
    };

    const std::string apAddr =
        apResolver->fetchFirstApAddress();

    if (apAddr.empty()) {
        ESP_LOGE(
            "cspot_session",
            "Spotify AP Resolver lieferte eine leere Adresse"
        );

        throw std::runtime_error(
            "Spotify AP address is empty"
        );
    }

    ESP_LOGI(
        "cspot_session",
        "Connecting with AP <%s>",
        apAddr.c_str()
    );

    connection->connect(apAddr);
