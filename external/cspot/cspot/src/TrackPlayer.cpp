...
if (track->state != QueuedTrack::State::READY) {
  CSPOT_LOG(
      info,
      "Track ist noch nicht bereit, aktueller Zustand=%d",
  ...
  while (!pendingReset &&
    ...
    if (track->state != QueuedTrack::State::READY &&
      ...
      CSPOT_LOG(
          info,
          "Warte weiter auf Track, Zustand=%d",
          static_cast<int>(track->state)
      );
    }
  }
  ...
  if (pendingReset) {
    CSPOT_LOG(
        info,
        "Track-Ladevorgang wegen Reset abgebrochen"
    );
    continue;
  }
  ...
  if (track->state != QueuedTrack::State::READY) {
    CSPOT_LOG(
        error,
        "Track konnte endgueltig nicht geladen werden, Zustand=%d",
        static_cast<int>(track->state)
    );
